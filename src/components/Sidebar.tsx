...
import { MdArrowBack, MdKeyboardArrowDown, MdWindow } from "react-icons/md";
import SidebarItem, { ISidebarItemProps } from "./SidebarItem";
import SidebarGroupChildItem from "./SidebarGroupChildItem";
import SidebarGroupParent, {
  ISidebarGroupParentProps,
} from "./SidebarGroupParent";
...
export type TAccess = "SuperAdmin" | "Admin" | "Employee" | "Manager" | "ALL";

interface ISimple extends ISidebarItemProps {
  type: "simple";
  access: TAccess[];
}
...
type TSidebarItems = (ISimple | INested)[];

const sidebarItems: TSidebarItems = [
  {
    type: "nested",
    access: ["ALL"],
    groupIcon: <MdWindow className="h-6 w-6" />,
    groupTitle: "Student Info.",
    groupPath: "/student-info",
    groupItems: [
      {
        title: "Bangladeshi Student",
        path: "/student-info/bangladeshi-student",
      },
      {
        title: "Foreign Student",
        path: "/student-info/foreign-student",
      },
    ],
  },
  {
    type: "simple",
    access: ["ALL"],
    icon: <TbFlag3 className="h-6 w-6" />,
    title: "Progress",
          ...
          <div>
            ...
            <ul className="mb-6 flex flex-col gap-1.5">
              ...
              {sidebarItems?.map((sidebarItem) => {
                if (sidebarItem?.type === "simple") {
                  const { icon, matchPath, path, title } = sidebarItem;
                  return (
                    <SidebarItem
                      icon={icon}
                      matchPath={matchPath}
